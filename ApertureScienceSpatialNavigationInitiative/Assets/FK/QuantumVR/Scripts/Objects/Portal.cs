...
namespace FK.QuantumVR.Objects
{
	...
	public class Portal : CompressedSpaceEntity
	{
		...
		private Vector3 InitCollider()
		{
			...
			col.isTrigger = true;
			Vector3 size = new Vector3(1, 1, SettingsLoader.Settings[SettingsLoader.PORTAL_COLLIDER_SIZE].FloatValue);
			col.size = size;
		...
		private void Update()
		{
			...
			for (int i = 0; i < _trackedObjects.Count; ++i)
			{
				...
				if ( !Single.IsNaN(_trackedObjectPrevRelativePositions[i].z)&& _trackedObjectPrevRelativePositions[i].z <= 0.0f && currentRelativePos.z > 0.0f && _obb.Contains(trackedObjectTransform.position))
				{
